...
const Map = ({ main_data }) => {
  ...
  return (
    <>
      {state.lat && isMounted ? <MapContainer center={[state.lat, state.lon]} zoom={17} style={{ height: '500px', width: '100%' }} attributionControl={false}>
        <TileLayer
      attribution='&copy; <a href="https://www.openstreetmap.org/copyright">OpenStreetMap</a> contributors'
        url="https://tile.openstreetmap.org/{z}/{x}/{y}.png"
      />
        <Marker position={[state.lat, state.lon]} icon={L.icon({ iconUrl: 'https://unpkg.com/leaflet@1.7.1/dist/images/marker-icon.png' })}>
